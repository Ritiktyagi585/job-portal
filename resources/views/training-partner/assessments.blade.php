...
<body>
    <div class="layout">
        <aside class="sidebar"><a class="brand" href="/training-partner/dashboard"><img src="{{ asset('ofclogo1.png') }}" alt="OnlyFreshers"></a><nav class="menu">@foreach($menuItems as $item)<a class="menu-item {{ $activePage === $item['key'] ? 'active' : '' }}" href="{{ $item['url'] }}"><span class="menu-icon">{{ $item['icon'] }}</span><span>{{ $item['title'] }}</span></a>@endforeach</nav><div class="side-bottom"><div class="upgrade"><h3>Create Better<br>Assessments</h3><p>Build skill-based assessments and evaluate students effectively.</p><button class="btn">Create Assessment</button><div class="upgrade-art">📋</div></div></div></aside>
        <!-- End Sidebar Section -->
